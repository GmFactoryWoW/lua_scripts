...
CREATE TRIGGER IF NOT EXISTS `paragon_config_statistics_before_insert`
BEFORE INSERT ON `paragon_config_statistic`
FOR EACH ROW
BEGIN
...
-- Validates that type_value matches the selected type
CREATE TRIGGER IF NOT EXISTS `paragon_config_statistics_before_update`
BEFORE UPDATE ON `paragon_config_statistic`
FOR EACH ROW
BEGIN
    DECLARE v_type VARCHAR(50);
    DECLARE v_value VARCHAR(50);
